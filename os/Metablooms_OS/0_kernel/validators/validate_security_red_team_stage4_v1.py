...
def main():
 ...
 p=bounded_subprocess_run([sys.executable,"-S",str(r/"0_kernel/scripts/security_red_team_runner_v1.py"),"--json"],cwd=str(r),text=True,capture_output=True,timeout=30)
 ...
 outp=r/"runtime/evals/security/SECURITY_RED_TEAM_STAGE4_VALIDATION_LATEST.json"; outp.parent.mkdir(parents=True,exist_ok=True); _mb_write_json_file(outp, out, operation_id='STAGE4_ATOMIC_JSON_0_kernel_validators_validate_security_red_team_stage4_v1_py_L44', create_parent=True, allowed_roots=[str(_MBAJWPath('/mnt/data').resolve())], indent=2, sort_keys=True, ensure_ascii=True, max_bytes=20000000)
