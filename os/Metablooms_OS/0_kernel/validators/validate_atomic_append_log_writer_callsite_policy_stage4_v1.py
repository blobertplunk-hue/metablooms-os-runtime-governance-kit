...
POLICY = ROOT / "0_kernel/registry/tool_governance/ATOMIC_APPEND_LOG_WRITER_CALLSITE_POLICY_STAGE4_v1.json"
EXCLUDE_PARTS = (
    "runtime/receipts/", "runtime/stage_bundles/", "runtime/tmp/", "__pycache__",
    ".stage3_append_bak_", ".stage4_bak_"
)
WARNING_EXEMPT = {
...
def scan():
    ...
    for p in files:
        ...
        for n in ast.walk(tree):
            if not isinstance(n, ast.Call) or not _is_append_open_call(n):
                continue
            if rel in CANONICAL_FILES:
                classification = "canonical_append_writer_internal"
            elif rel in WARNING_EXEMPT:
                classification = WARNING_EXEMPT[rel]
            else:
                classification = "blocking_live_direct_append_open"
...
def main():
    ...
    rows = scan()
    blocking = [r for r in rows if r.get("classification") in {"blocking_live_direct_append_open", "parse_error"}]
    warnings = [r for r in rows if r not in blocking]
    out = {
        "artifact_type": "AtomicAppendLogWriterCallsitePolicyStage4Result.v1",
