...
def main():
    ...
    smoke=None
    if planner.exists():
      cp=bounded_subprocess_run(['python3','-S',str(planner),'--stage','AGENT_HARNESS_STAGE1_VALIDATOR_SMOKE','--write-plan','--json'],cwd=str(r),text=True,capture_output=True,timeout=30)
    ...
    _mb_write_json_file(r / 'runtime/evals/agent_harness/AGENT_HARNESS_STAGE1_VALIDATION_LATEST.json', out, operation_id='STAGE4_ATOMIC_JSON_0_kernel_validators_validate_agent_harness_stage1_v1_py_L52', create_parent=True, allowed_roots=[str(_MBAJWPath('/mnt/data').resolve())], indent=2, sort_keys=True, ensure_ascii=True, max_bytes=20000000)
