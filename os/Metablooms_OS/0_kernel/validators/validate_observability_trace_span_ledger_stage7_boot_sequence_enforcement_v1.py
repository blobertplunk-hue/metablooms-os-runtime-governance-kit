...
def main(argv=None):
    ...
    if not issues:
        ...
        ptr=ptrs[0]
        allowed_pointer_stages={STAGE,'OBSERVABILITY_TRACE_SPAN_LEDGER_STAGE8_BOOT_RECEIPT_WRAPPER_ONLY_ENFORCEMENT_AND_HISTORICAL_CALLSITE_QUARANTINE'}
        if ptr.get('stage_id') not in allowed_pointer_stages or ptr.get('last_stage') not in allowed_pointer_stages: issues.append('pointer_not_stage7_or_stage8_superset')
