...
def main():
 ...
 cp=bounded_subprocess_run(['python3','-S',str(r/'0_kernel/scripts/agent_harness_executor_review_gate_v1.py'),'--stage','IMPLEMENT_AGENT_HARNESS_STAGE_3_EXECUTOR_REVIEW_GATE','--json'],cwd=str(r),text=True,capture_output=True,timeout=30)
 ...
 out=r/'runtime/evals/agent_harness/AGENT_HARNESS_STAGE3_VALIDATION_LATEST.json'; out.parent.mkdir(parents=True,exist_ok=True); _mb_write_json_file(out, result, operation_id='STAGE4_ATOMIC_JSON_0_kernel_validators_validate_agent_harness_stage3_v1_py_L37', create_parent=True, allowed_roots=[str(_MBAJWPath('/mnt/data').resolve())], indent=2, sort_keys=True, ensure_ascii=True, max_bytes=20000000)
