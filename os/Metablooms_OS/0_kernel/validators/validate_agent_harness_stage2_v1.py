...
def main():
    ...
    if planner.exists():
        cp=bounded_subprocess_run(['python3','-S',str(planner),'--stage','AGENT_HARNESS_STAGE2_VALIDATOR_SMOKE','--write-plan','--parallel-plan','--json'],cwd=str(r),text=True,capture_output=True,timeout=60)
    ...
    if diff.exists():
        cp=bounded_subprocess_run(['python3','-S',str(diff),'--stage','AGENT_HARNESS_STAGE2_VALIDATOR_SMOKE','--json'],cwd=str(r),text=True,capture_output=True,timeout=60)
    ...
    _mb_write_json_file(r / 'runtime/evals/agent_harness/AGENT_HARNESS_STAGE2_VALIDATION_LATEST.json', out, operation_id='STAGE4_ATOMIC_JSON_0_kernel_validators_validate_agent_harness_stage2_v1_py_L57', create_parent=True, allowed_roots=[str(_MBAJWPath('/mnt/data').resolve())], indent=2, sort_keys=True, ensure_ascii=True, max_bytes=20000000)
