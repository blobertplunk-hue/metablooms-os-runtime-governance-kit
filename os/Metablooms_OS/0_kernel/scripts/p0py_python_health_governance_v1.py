...
def write_json_atomic(path: Path, data: Dict[str, Any]) -> None:
    ...
    _mb_write_json_file(tmp, data, operation_id='STAGE4_ATOMIC_JSON_0_kernel_scripts_p0py_python_health_governance_v1_py_L75', create_parent=True, allowed_roots=[str(_MBAJWPath('/mnt/data').resolve())], indent=2, sort_keys=True, ensure_ascii=True, max_bytes=20000000)
    os.replace(tmp, path)
...
def run_timed(cmd: List[str], timeout_s: int) -> Dict[str, Any]:
    ...
    try:
        proc = bounded_subprocess_run(cmd, text=True, capture_output=True, timeout=timeout_s)
