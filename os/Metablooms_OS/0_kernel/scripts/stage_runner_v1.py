...
def run_checkpoint(root, args):
    ...
    pr=bounded_subprocess_run(['python3','-S',str(cm),*args],cwd=str(root),text=True,capture_output=True,timeout=60)
...
def main(argv=None):
    ...
    if breaker.exists():
        ...
        br=bounded_subprocess_run(br_cmd,cwd=str(root),text=True,capture_output=True,timeout=min(max(a.timeout,5),20))
    ...
    gp=bounded_subprocess_run(['python3','-S',str(gate),'--work-order',str(wp),'--fixtures','--json'],cwd=str(root),text=True,capture_output=True,timeout=min(a.timeout,90))
    ...
    pr=bounded_subprocess_run(['python3','-S',str(exe),'--work-order',str(wp),'--security-gate-passed'],cwd=str(root),text=True,capture_output=True,timeout=a.timeout)
    try: payload=json.loads(pr.stdout.strip() or '{}')
    except Exception: payload={'stdout_tail':pr.stdout[-4000:],'stderr_tail':pr.stderr[-2000:]}
    # WC12_RUNTIME_EVAL_EXIT_GATE_WIRING
    # Successful delegated stages must prove the runtime eval gate and continuation prompt gate before PASS.
    if pr.returncode == 0:
        gate_binding = root/'0_kernel/registry/world_class_gap_plan/RUNTIME_ORCHESTRATOR_EXIT_GATE_WIRING_v1.json'
        gate_script = root/'0_kernel/evals/real_task_eval_harness/real_task_eval_runtime_gate_v1.py'
        if gate_binding.exists():
            if not gate_script.exists():
                return emit({'verdict':'STAGE_RUNNER_EXIT_GATE_MISSING','root':str(root),'stage_name':a.stage_name,'required_gate':str(gate_script),'fail_closed':True,'created_utc':utc_now()},31)
            gp = bounded_subprocess_run(['python3','-S',str(gate_script),str(root)],cwd=str(root),text=True,capture_output=True,timeout=min(a.timeout,120))
            try: gate_payload=json.loads(gp.stdout.strip() or '{}')
            except Exception: gate_payload={'parse_error':True,'stdout_tail':gp.stdout[-4000:],'stderr_tail':gp.stderr[-2000:]}
            if gp.returncode != 0 or gate_payload.get('verdict') != 'PASS':
                return emit({'verdict':'STAGE_RUNNER_EXIT_GATE_BLOCK','root':str(root),'stage_name':a.stage_name,'exit_gate_returncode':gp.returncode,'exit_gate':gate_payload,'fail_closed':True,'created_utc':utc_now()},31)
