...
def write_json(path: Path, obj):
    ...
    _mb_write_json_file(path, obj, operation_id='STAGE4_ATOMIC_JSON_0_kernel_scripts_run_governance_regression_suite_v1_py_L38', create_parent=True, allowed_roots=[str(_MBAJWPath('/mnt/data').resolve())], indent=2, sort_keys=True, ensure_ascii=True, max_bytes=20000000)
...
def run_case(case):
    ...
    start=time.time()
    try:
        p=bounded_subprocess_run(cmd, cwd=str(ROOT), text=True, capture_output=True, timeout=case.get('timeout_s', 12))
        timed_out=False
        rc=p.returncode
        stdout=p.stdout
        stderr=p.stderr
