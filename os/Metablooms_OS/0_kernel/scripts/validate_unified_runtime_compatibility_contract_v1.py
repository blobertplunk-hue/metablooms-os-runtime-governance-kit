...
def main() -> int:
    ...
    if redirect_path.is_file():
        try:
            ...
            machine_status = redirect.get("machine_use_status")
            redirect_lifecycle_status = redirect.get("status")
            accepted_machine_statuses = {EXPECTED_STATUS, "forbidden_as_runtime_authority"}
            accepted_lifecycle_statuses = {EXPECTED_STATUS, "archive_only_after_r3_confirmed_after_r11"}
            redirect_ok = (machine_status in accepted_machine_statuses and redirect_lifecycle_status in accepted_lifecycle_statuses and redirect.get("successor_contract") == "runtime/governance/contracts/unified_runtime_compatibility_contract_v1.json")
        except Exception:
            redirect_ok = False
