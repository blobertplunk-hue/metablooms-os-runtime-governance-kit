...
_EXECUTION_LIB = Path(__file__).resolve().parents[2] / "lib" / "execution"
...
def _run(cmd: list[str], timeout_seconds: int = 8) -> subprocess.CompletedProcess[str]:
    return run_compat(
        ...
        operation_type="tracker_stage_finalization_subprocess",
        stage="TRACKER_STAGE_FINALIZATION_HOOK_v1",
    )
...
def _write_json_atomic(path: Path, obj: Dict[str, Any]) -> None:
    write_json_file(
        path,
        obj,
        operation_id="tracker_stage_finalization_atomic_write",
