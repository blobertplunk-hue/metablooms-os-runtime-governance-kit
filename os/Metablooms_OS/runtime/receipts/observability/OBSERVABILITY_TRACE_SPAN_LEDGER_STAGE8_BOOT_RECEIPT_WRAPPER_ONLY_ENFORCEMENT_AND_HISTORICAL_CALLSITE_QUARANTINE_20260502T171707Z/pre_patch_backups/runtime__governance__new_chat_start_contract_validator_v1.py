...
def validate_new_chat_start_contract(root: str|Path):
    ...
    if not errors:
        ...
        for phrase in ['Verify the authority ZIP SHA-256','/mnt/data/Metablooms_OS','boot_critical_governance_loader_v1.py','governance_scatter_prevention_v1.py','fresh_chat_boot_rehearsal_v1.py','prompt_governance_v1','runtime_starter_smoke_contract_wrapper_v1.py']:
            if phrase not in contract: errors.append('contract_missing_phrase:'+phrase)
        if 'Run `runtime/governance/runtime_starter_smoke_v1.py`' in contract or 'Run runtime/governance/runtime_starter_smoke_v1.py.' in contract:
            errors.append('contract_uses_direct_runtime_starter_smoke_operator_command')
