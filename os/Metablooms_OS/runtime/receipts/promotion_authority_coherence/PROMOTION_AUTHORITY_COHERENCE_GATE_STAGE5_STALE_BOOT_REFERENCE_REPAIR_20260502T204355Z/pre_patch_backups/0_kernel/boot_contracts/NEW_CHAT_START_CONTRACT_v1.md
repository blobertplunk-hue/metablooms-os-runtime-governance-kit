...
## OBSERVABILITY TRACE/SPAN LEDGER STAGE 9 — LIVE BOOT GUIDANCE + TRACKER DEEP LINKS

Current live boot guidance is extracted into these current-only artifacts:

- `runtime/state/operator_surface/LIVE_BOOT_GUIDANCE_LATEST.json`
- `runtime/state/operator_surface/LIVE_BOOT_GUIDANCE_LATEST.md`

Live boot guidance remains wrapper-only. Run:

```bash
python runtime/governance/runtime_starter_smoke_contract_wrapper_v1.py --root /mnt/data/Metablooms_OS --json
```

Before trusting the current live boot panel or deep links, run:

```bash
python 0_kernel/validators/validate_observability_trace_span_ledger_stage9_live_boot_guidance_v1.py --root /mnt/data/Metablooms_OS --json
```

## Stage 10 mobile/minimal boot surface

Open `OPEN_OPERATOR_VISUAL_TRACKER.html` first. It now defaults to minimal mode with one current action, wrapper-only boot smoke guidance, current authority state, and expandable evidence drawers. Minimal-mode source: `runtime/state/operator_surface/BOOT_SURFACE_MINIMAL_MODE_LATEST.json`.
