...
def cmd(c,cwd=None,timeout=10):
 ...
 try:
  p=subprocess.run(c,cwd=cwd,text=True,capture_output=True,timeout=timeout)
...
def main():
 ...
 if a.write_report: Path(a.write_report).parent.mkdir(parents=True,exist_ok=True); Path(a.write_report).write_text(json.dumps(r,indent=2,sort_keys=True)+'\n',encoding='utf-8')
