...
def main(argv=None):
 ...
 if not a.security_gate_passed:
  ...
  gp=subprocess.run(['python3','-S',str(gate),'--work-order',str(wp),'--fixtures','--json'],cwd=str(r),text=True,capture_output=True,timeout=90)
 ...
 if review_gate.exists():
  rg=subprocess.run(['python3','-S',str(review_gate),'--stage',stage,'--work-order',str(wp),'--strict','--json'],cwd=str(r),text=True,capture_output=True,timeout=60)
  if rg.returncode!=0:
 ...
 if match.get('handler_id')=='evals_trace_review_stage4_builtin':
  ...
  gr=subprocess.run(['python3','-S',str(gate),'--json'],cwd=str(r),text=True,capture_output=True,timeout=90) if gate.exists() else None
  vp=subprocess.run(['python3','-S',str(validator)],cwd=str(r),text=True,capture_output=True,timeout=90) if validator.exists() else None
 ...
 if match.get('handler_id')=='evals_trace_review_stage3_builtin':
  ...
  rr=subprocess.run(['python3','-S',str(runner),'--json'],cwd=str(r),text=True,capture_output=True,timeout=90) if runner.exists() else None
  vp=subprocess.run(['python3','-S',str(validator)],cwd=str(r),text=True,capture_output=True,timeout=90) if validator.exists() else None
 ...
 if match.get('handler_id')=='state_checkpoint_resume_interrupt_stage2_builtin':
  ...
  vp=subprocess.run(['python3','-S',str(validator)],cwd=str(r),text=True,capture_output=True,timeout=120) if validator.exists() else None
 ...
 if match.get('handler_id')=='agent_harness_stage1_builtin':
  ...
  pr=subprocess.run(['python3','-S',str(planner),'--stage',stage,'--write-plan','--json'],cwd=str(r),text=True,capture_output=True,timeout=45) if planner.exists() else None
  vp=subprocess.run(['python3','-S',str(validator)],cwd=str(r),text=True,capture_output=True,timeout=45) if validator.exists() else None
