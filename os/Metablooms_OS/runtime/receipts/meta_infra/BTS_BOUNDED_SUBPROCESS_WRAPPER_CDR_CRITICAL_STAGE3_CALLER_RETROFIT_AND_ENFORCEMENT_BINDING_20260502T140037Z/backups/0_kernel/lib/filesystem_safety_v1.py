...
def df_record(path:Path):
    ...
    p=subprocess.run(["df","-P",str(target)],text=True,capture_output=True,timeout=10)
...
def atomic_write_json(path:Path,payload:dict):
    ...
    fd,tmp_name=tempfile.mkstemp(prefix=f".{path.name}.",suffix=".tmp",dir=str(path.parent)); tmp=Path(tmp_name)
    try:
        with os.fdopen(fd,"w",encoding="utf-8") as f:
            f.write(json.dumps(payload,indent=2,sort_keys=True)+"\n"); f.flush(); os.fsync(f.fileno())
        pre=atomic_replace_preflight(tmp,path)
        if pre["verdict"]!="ATOMIC_REPLACE_SAFE": return {"verdict":"ATOMIC_WRITE_BLOCK","path":str(path),"preflight":pre}
        os.replace(tmp,path)
        try:
            dfd=os.open(str(path.parent),os.O_DIRECTORY); os.fsync(dfd); os.close(dfd)
        except Exception as e: pre.setdefault("warnings",[]).append({"directory_fsync_warning":str(e)})
