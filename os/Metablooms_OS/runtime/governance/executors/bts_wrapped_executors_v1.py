...
def run_json_command(cmd: List[str], cwd: Optional[Path] = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    result = bounded_subprocess_run(cmd, cwd=str(cwd) if cwd else None, capture_output=True, text=True, timeout=timeout)
...
def run_node_json(cmd: List[str], cwd: Path, timeout: int = 120) -> Dict[str, Any]:
    ...
    result = bounded_subprocess_run(cmd, cwd=str(cwd), capture_output=True, text=True, timeout=timeout, env=env)
    if result.returncode != 0:
