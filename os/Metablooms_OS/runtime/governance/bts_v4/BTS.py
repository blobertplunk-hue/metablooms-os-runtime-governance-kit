...
_MB_SELF_IO = Path(__file__).resolve()
for _MB_PARENT_IO in [_MB_SELF_IO] + list(_MB_SELF_IO.parents):
    _MB_IO_LIB = _MB_PARENT_IO / "0_kernel" / "lib" / "io"
    if (_MB_IO_LIB / "atomic_append_log_compat_v1.py").exists():
        if str(_MB_IO_LIB) not in sys.path:
            sys.path.insert(0, str(_MB_IO_LIB))
        break
...
def _run_pre_action_gate(os_root: Path, envelope: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = bounded_subprocess_run(['node', str(gate), str(env_path)], capture_output=True, text=True, cwd=str(os_root), timeout=30)
...
class BTS:
    ...
    def _append_to_log(self, entry: Dict) -> None:
        ...
        # Normalize before serialization so the line hash chained in BTS is
        # exactly the line emitted by the governed append-log writer.
        entry.setdefault("schema_version", str(entry.get("schema") or SCHEMA_VERSION))
        entry.setdefault("event_id", str(entry.get("turn_id") or entry.get("decision_id") or f"bts_{_compact_ts()}"))
        entry.setdefault("timestamp_utc", str(entry.get("timestamp") or _utc_iso()))
        entry.setdefault("source", "BTS.v4")
        entry.setdefault("event_type", str(entry.get("type") or "BTS_LOG_ENTRY"))
        entry.setdefault("severity", "info")
        entry.setdefault("payload", {"bts_record_keys": sorted(str(k) for k in entry.keys())})
        line = json.dumps(entry, sort_keys=True, separators=(",", ":"), ensure_ascii=False, allow_nan=False)
        _mb_append_jsonl_record(
            self._log_path,
            entry,
            operation_id=f"bts_append_{entry.get('event_id')}_{_compact_ts()}",
            allowed_roots=[str(self._root.resolve())],
            receipt_dir=self._receipts_dir / "append_log_writer",
            source="BTS.v4",
            event_type=str(entry.get("event_type") or "BTS_LOG_ENTRY"),
            severity=str(entry.get("severity") or "info"),
            durability_mode="sync_on_critical",
            allowed_suffixes=[".ndjson", ".jsonl", ".log.jsonl", ".ledger.jsonl", ".events.jsonl"],
        )
